...
factura
N° - ' . str_pad($model->Codigo_Cliente, 10, "0", STR_PAD_LEFT))), 1, 'C');

    }

    function ImpresionCuerpo($model,$Direccion)
    {
        ...
        $this->Cell(11.65, 1, utf8_decode($modelo->NombreCliente($model->Codigo_Cliente)), 0, 'R');
//        $this->Line(14.5,5.2,2.8,5.2);
        ...
        $this->SetFont('Arial', '', 13);
        $this->Cell(10.95, 1, utf8_decode($Direccion), 0, 'R');


        $this->SetFont('Arial', 'B', 15);
        $this->Cell(5, 1, utf8_decode('Estado del Pago:'), 0, 'C');
        $this->SetFont('Arial', '', 13);
        $this->Cell(8.75, 1, utf8_decode('Pendiende'), 0, 'R');
        $this->Ln(1.5);
        ...
        $sqlStatement = "
        SELECT df.Cantidad,df.Descripcion,df.precio,(df.Cantidad*df.precio) as Total
        FROM factura f
        INNER JOIN d_factura df on f.id = df.factura
        WHERE f.id = '" . $model->Codigo_Cliente . "' ORDER BY 1 ASC ;";
        $comando = $connection->createCommand($sqlStatement);
        $resultado = $comando->query();

        while ($row = $resultado->read()) {
            $this->SetFont('Arial', '', 12);
            $this->Cell(3, 0.5, utf8_decode($row['Cantidad']), 1, '', 'C');
            $this->Cell(18.5, 0.5, utf8_decode($row['Descripcion']), 1, '', 'L');
            $this->Cell(3, 0.5, utf8_decode($row['precio']), 1, '', 'C');
            $this->Cell(3, 0.5, utf8_decode($row['Total']), 1, '', 'C');
            $this->Ln();
        }
    }
    ...
    function ImpresionFooter($model)
    {
        ...
        $this->Ln();
        $this->Cell(21.5, 0.7, utf8_decode(strtoupper('son: '. $this->numtoletras($model->Total))), 1, '', 'L');
        $this->Cell(6, 0.7, '', 1, '', 'L');
        $this->Ln();

        $this->Cell(21.5, 1.65, '', 1, '', 'L');
        $this->Cell(3, 0.55, strtoupper('Sub-Total'), 1, '', 'C');
        $this->Cell(3, 0.55, strtoupper($model->Subtotal), 1, '', 'C');
        $this->Ln();

        $this->Cell(21.5, 0.55, '', 0, '', 'L');
        $this->Cell(3, 0.55, strtoupper('I.G.V. %'), 1, '', 'C');
        $this->Cell(3, 0.55, strtoupper($model->igv), 1, '', 'C');
        $this->Ln();

        $this->Cell(21.5, 0.55, '', 0, '', 'L');
        $this->Cell(3, 0.55, strtoupper('Total'), 1, '', 'C');
        $this->Cell(3, 0.55, strtoupper($model->Total), 1, '', 'C');
        $this->Ln();
    }
...
//
    ...
    function Impresion($model,$Direccion)
    {

        $this->ImpresionCabezara($model);
        $this->ImpresionCuerpo($model,$Direccion);
        $this->ImpresionFooter($model);
    }
}

$pdf = new PDF('L', 'cm', 'A4');
$Reporte = "Factura-" . str_pad($model->Codigo_Cliente, 10, "0", STR_PAD_LEFT) . ".pdf";
$pdf->AddPage();
$pdf->Impresion($model,$Direccion);
$pdf->SetTitle("Factura - " . str_pad($model->Codigo_Cliente, 10, "0", STR_PAD_LEFT));
